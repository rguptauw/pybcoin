...
"""
This module creates the UI for the App using Dash. 
"""
...
w3_css_url = "https://www.w3schools.com/w3css/4/w3.css"
my_css_url = "../static/mycss.css"
plotly_url = "https://cdn.plot.ly/plotly-latest.min.js"
data_path = "../data/BitcoinPrice.csv"
up_arrow_img_src = "static/uparrow_anim.gif"
down_arrow_img_src = "static/downarrow_anim.gif"
wc_url = "static/date_"

app.css.append_css({
    'external_url' : w3_css_url
})
app.scripts.append_script({'external_url' : plotly_url})

timeseries_df = pd.read_csv(data_path).dropna()
...
if going_up == 1:
    ...
    img_src = up_arrow_img_src
else:
    color_style_class = 'w3-text-red'
    img_src = down_arrow_img_src
...
def update_global_vars(fr_dt, t_dt):
    """
    This function updates all the global variables so that the different Dash components can refer the updated data.
    The data gets updated based on the new date range passed
...
app.layout = html.Div(children = [ #Main Container
    html.Link(href = my_css_url, rel = 'stylesheet'),
...
def update_slider_parent(fr_dt, t_dt):
    """
    This callback function updates the slider range based on date picker component.
    """
    fr_dt = fr_dt[:10]
    t_dt = t_dt[:10]
...
def update_timeseries(fr_dt, t_dt):
    """
    This callback function updates the time series based on date picker component.
    """
    fr_dt = fr_dt[:10]
    t_dt = t_dt[:10]
    from_dt = datetime.datetime.strptime(fr_dt, '%Y-%m-%d')
...
def update_wordcloud(dt):
    """
    This callback function updates the wordcloud based on the date picked in the slider.
    """
    pos = str(dt)
    fname = wc_url + marks_vals[pos][:4] + "-" + marks_vals[pos][5:7] + "-" + marks_vals[pos][8:10] + '.png'
    return fname
...
def update_wcheader(dt):
    """
    This callback function updates the wordcloud header based on the date picked in the slider.
    """
    pos = str(dt)
...
def update_slider_leftval(fr_dt):
    """
    This callback function updates the left label of slider based on the date picked in the slider.
    """
...
def update_slider_rightval(t_dt):
    """
    This callback function updates the right label of slider based on the date picked in the slider.
    """
